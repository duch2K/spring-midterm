...
import iitu.midterm.demo.controllers.StudentController;
...
public class Library {
    ...
    private void login() {
        ...
        while(true) {
            ...
            if (studentController.login(username, password)) {
                student = studentController.getStudent(username);
    ...
    private void showMyBorrowedBooks() {
        List<Book> books = bookController.getBooksByStudent(student);
    ...
    private void addBookInBorrowedBooks() {
        List<Book> books = bookController.getBooksByNotStudent(student);
        ...
        if (books.size() != 0) {
            ...
            studentController.addBookInBorrowedBooks(student.getUsername(), books.get(number));
        } else {
    ...
    public void deleteBookInBorrowedBooks() {
        ...
        if (books.size() != 0) {
            ...
            studentController.deleteBookInBorrowedBooks(student.getUsername(), books.get(number));
        } else {
            System.out.println("\nYour borrowed books list is empty!");
        }
    }
